...
export function TemplateGallery({ selectedTemplateId, onSelect }: TemplateGalleryProps) {
  ...
  const [orgTemplates, setOrgTemplates] = useState<SharedTemplate[]>([])

  useEffect(() => {
    fetch("/api/dashboard/templates")
      .then((res) => res.ok ? res.json() : [])
      .then((data) => setOrgTemplates(Array.isArray(data) ? data.filter((t: SharedTemplate) => t.category !== "pipeline") : []))
      .catch(() => {})
  }, [])
