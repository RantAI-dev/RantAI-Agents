...
async function gatewayMode() {
  ...
  if (fs.existsSync(configPath)) {
    ...
    const output = []
    let currentSection = ""
    const skipSections = new Set(["gateway", "autonomy"])
    let skipping = false
...
`
  ...
  // 9. Spawn gateway
  console.log(`[Gateway] Spawning ${CLAW_BIN} gateway on port 8080...`)
  const rantaiclaw = spawn(CLAW_BIN, ["gateway", "--port", "8080", "--host", "0.0.0.0"], {
    stdio: "inherit",
  ...
  rantaiclaw.on("exit", (code) => {
    console.log(`[Gateway] Gateway exited with code ${code}`)
    process.exit(code || 0)
  })
