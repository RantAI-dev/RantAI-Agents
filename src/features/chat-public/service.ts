...
export interface ChatPublicServiceError {
  status: number
  error: string
  code?: string
}

export function isChatPublicServiceError(
  value: unknown
): value is ChatPublicServiceError {
...
export async function runChat(params: {
  ...
  try {
    ...
    const headerAssistantId = params.headers.assistantId
    const headerSystemPromptB64 = params.headers.systemPromptB64
    const headerUseKnowledgeBase = params.headers.useKnowledgeBase

    // Decode base64 system prompt from header
    let headerSystemPrompt: string | null = null
    if (headerSystemPromptB64) {
      try {
        headerSystemPrompt = Buffer.from(headerSystemPromptB64, "base64").toString(
          "utf-8"
        )
      } catch {
    ...
    debug(
      "Body systemPrompt:",
      body.systemPrompt ? body.systemPrompt.substring(0, 40) + "..." : "null"
    )
        ...
        if (dbAssistant) {
          ...
          if (dbAssistant.memoryConfig && typeof dbAssistant.memoryConfig === "object") {
            ...
            assistantMemoryConfig = {
              ...
              memoryInstructions:
                typeof mc.memoryInstructions === "string"
                  ? mc.memoryInstructions
                  : undefined,
            }
          }
    ...
    if (assistantId) {
      try {
        const chatflowWorkflow = await findActiveChatflowByAssistantId(assistantId)
    ...
    if (fileDocumentIds && Array.isArray(fileDocumentIds) && fileDocumentIds.length > 0) {
      try {
        ...
        const fileDocs = await findDocumentsByIds(fileDocumentIds)
    ...
    if (body.enableTools === false && !shouldForceToolsForSkills) {
      ...
      if (missingToolNames.length > 0) {
        const onDemand = await resolveToolsByNames(
          missingToolNames,
          assistantId ?? null,
          modelId,
          {
        ...
        for (const name of onDemand.toolNames) {
...
export async function uploadChatAttachment(params: {
  ...
  try {
    if (!ALLOWED_TYPES.includes(params.file.type)) {
      ...
      } satisfies ChatPublicServiceError
    }
...
export async function getChatAttachment(fileId: string) {
  try {
    const { buffer, contentType } = await readChatAttachment(fileId)
    ...
    if ((error as NodeJS.ErrnoException).code === "ENOENT") {
      return { status: 404, error: "File not found" } satisfies ChatPublicServiceError
    }
...
export async function listConversations() {
  try {
    return await findActiveConversations()
  } catch (error) {
...
export async function createConversationRecord(params: { sessionId: string }) {
  try {
    return await createConversation(params.sessionId)
  } catch (error) {
...
export async function listConversationMessages(params: { conversationId: string }) {
  try {
    const messages = await findConversationMessages(params.conversationId)
    return {
...
export async function getConversationStatus(params: { conversationId: string }) {
  try {
    const conversation = await findConversationStatus(params.conversationId)
    if (!conversation) {
      return { status: 404, error: "Not found" } satisfies ChatPublicServiceError
    }
