...
import {
  ...
  type AssistantEditorKnowledgeGroup,
  type AssistantEditorToolItem,
...
interface AssistantEditorProps {
  ...
  initialHydrationData?: AssistantEditorHydrationData | null
}
...
export function AssistantEditor({
  ...
  const [availableTools, setAvailableTools] = useState<AssistantEditorToolItem[]>([])
  const [groups, setGroups] = useState<AssistantEditorKnowledgeGroup[]>([])
  const [selectedToolIds, setSelectedToolIds] = useState<string[]>([])
  const [loadingTools, setLoadingTools] = useState(false)
  const [loadingGroups, setLoadingGroups] = useState(false)
  const [hydrationData] = useState<AssistantEditorHydrationData | null>(() =>
    initialHydrationData ?? readHydrationPayload<AssistantEditorHydrationData>("rantai-assistant-editor-hydration")
  )
  ...
  const syncAssistantEditorData = useCallback(
    (currentAssistant: Assistant | null | undefined) => {
      if (hydrationData) {
        setAvailableTools(hydrationData.availableTools)
        setGroups(hydrationData.knowledgeGroups)
        setLoadingTools(false)
        setLoadingGroups(false)
      }

      if (currentAssistant?.id && hydrationData?.assistantId === currentAssistant.id) {
        setSelectedToolIds(hydrationData.selectedToolIds)
        return
      }
      ...
      if (currentAssistant?.id) {
        void fetchAssistantTools(currentAssistant.id)
      }
    },
    [fetchAssistantTools, fetchGroups, fetchTools, hydrationData]
  )
  ...
  useEffect(() => {
    if (open) {
      ...
      syncAssistantEditorData(assistant)
    }
  }, [open, assistant, syncAssistantEditorData])
