...
function getToolsLabel(mode: ToolMode, selectedCount: number, totalCount: number): string {
  ...
  return `Tools: Auto (${selectedCount}/${totalCount})`
}
...
function getSkillsLabel(mode: SkillMode, selectedCount: number, totalCount: number): string {
  ...
  return `Skills: Auto (${selectedCount}/${totalCount})`
}
...
export const ChatInputToolbar = memo<ChatInputToolbarProps>(({
  ...
  return (
    <div className="flex items-center gap-0.5">
      ...
      <DropdownMenu onOpenChange={(open) => { if (open) onOpenToolsMenu?.() }}>
        <Tooltip>
          <TooltipTrigger asChild>
          ...
          <TooltipContent side="top">
            <p>{getToolsLabel(toolMode, toolMode === "auto" ? defaultToolNames.length : selectedToolNames.length, toolCount)}</p>
          </TooltipContent>
        </Tooltip>
        <DropdownMenuContent side="top" align="start" className="w-64">
          {/* Auto */}
          <DropdownMenuItem onClick={() => { onSetToolMode("auto"); onSetSelectedToolNames(defaultToolNames) }}>
            <Sparkles className="h-3.5 w-3.5 mr-2 text-muted-foreground" />
            <span className="flex-1">Auto</span>
            <span className="text-[10px] text-muted-foreground mr-2">Agent default</span>
          ...
          {assistantTools.map((t) => {
            ...
            const isSelected = toolMode === "select"
      ...
      <DropdownMenu onOpenChange={(open) => { if (open) onOpenSkillsMenu?.() }}>
          <Tooltip>
            <TooltipTrigger asChild>
              <DropdownMenuTrigger asChild>
                <Button
            ...
            <TooltipContent side="top">
              <p>{getSkillsLabel(skillMode, skillMode === "auto" ? defaultSkillIds.length : selectedSkillIds.length, skillCount)}</p>
            </TooltipContent>
          </Tooltip>
          <DropdownMenuContent side="top" align="start" className="w-64">
            {/* Auto */}
            <DropdownMenuItem onClick={() => { onSetSkillMode("auto"); onSetSelectedSkillIds(defaultSkillIds) }}>
              <Sparkles className="h-3.5 w-3.5 mr-2 text-muted-foreground" />
              <span className="flex-1">Auto</span>
              <span className="text-[10px] text-muted-foreground mr-2">Agent default</span>
            ...
            {assistantSkills.map((s) => {
              const isSelected = skillMode === "select"
                ? selectedSkillIds.includes(s.id)
                : defaultSkillIds.includes(s.id)
              return (
                <DropdownMenuItem
                  key={s.id}
                  onSelect={(e) => e.preventDefault()}
