...
import { AvatarPicker } from "@/src/features/assistants/components/builder/avatar-picker"
...
import { TemplateGallery } from "@/src/features/digital-employees/components/new/template-gallery"
...
export default function NewDigitalEmployeePageClient({
  preselectedGroupId,
  initialAssistants,
  initialGroups,
  initialTemplates,
  ...
  const { groups } = useEmployeeGroups({ initialGroups })
  ...
  const [orgTemplates] = useState<SharedTemplate[]>(initialTemplates)
    ...
    <div className="flex flex-col h-full">
        ...
        <motion.div variants={fadeUp} className="flex items-center gap-2">
          {STEPS.map((s, i) => (
            <div key={s.label} className="flex items-center gap-2">
              <button
                onClick={() => i < step && handleSetStep(i)}
                className={cn(
      ...
      <div className="flex-1 overflow-auto px-6 pb-6">
        ...
        <div className="flex items-center gap-3 mt-8 max-w-2xl">
          {step > 0 && (
            <Button variant="outline" onClick={() => handleSetStep(step - 1)}>
