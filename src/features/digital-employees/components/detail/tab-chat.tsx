...
export function TabChat({
  ...
  const handleSendQueuedMessages = useCallback(async () => {
    ...
    setQueuedMessages([])
  }, [containerRunning, employee.id, queuedMessages])
        ...
        <>
          {queuedMessages.length > 0 && (
            <div className="border-b px-4 py-2">
              <Button size="sm" variant="outline" onClick={() => void handleSendQueuedMessages()}>
                Send {queuedMessages.length} queued message{queuedMessages.length > 1 ? "s" : ""}
              </Button>
            </div>
          )}
          <ChatWorkspace
