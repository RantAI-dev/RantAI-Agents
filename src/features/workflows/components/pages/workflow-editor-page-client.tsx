...
import { WorkflowCanvas } from "@/src/features/workflows/components/workflow-canvas"
import { WorkflowToolbar } from "@/src/features/workflows/components/workflow-toolbar"
import { NodePalette } from "@/src/features/workflows/components/node-palette"
import { PropertiesPanel } from "@/src/features/workflows/components/properties-panel"
import { RunHistoryDialog } from "@/src/features/workflows/components/run-history-dialog"
import { KeyboardShortcutsDialog } from "@/src/features/workflows/components/keyboard-shortcuts-dialog"
import { QuickAddDialog } from "@/src/features/workflows/components/quick-add-dialog"
import { ChatTestPanel } from "@/src/features/workflows/components/chat-test-panel"
import { OnboardingOverlay } from "@/src/features/workflows/components/onboarding-overlay"
...
interface WorkflowEditorAssistantOption {
  id: string
  name: string
  emoji: string
}

interface WorkflowEditorCredentialOption {
  id: string
  name: string
  type: string
}

interface WorkflowEditorToolOption {
  id: string
  name: string
  displayName: string
  description: string
...
interface WorkflowEditorKnowledgeGroupOption {
  id: string
  name: string
  documentCount: number
}

interface WorkflowEditorPageClientProps {
  workflowId: string
...
export default function WorkflowEditorPageClient({
  ...
  const {
    ...
    deleteWorkflow,
    getWorkflowById,
  } = useWorkflows(undefined, { initialWorkflows })

  const workflow = workflows.find((w) => w.id === workflowId)
    ?? (initialWorkflow?.id === workflowId ? initialWorkflow : undefined)
  ...
  const {
    ...
    executeWorkflow,
    resumeRun,
  } = useWorkflowRuns(workflowId, { initialRuns })
  ...
  const [isResolvingWorkflow, setIsResolvingWorkflow] = useState(false)
  const prevRunStatusRef = useRef<string | null>(null)
  const workflowResolveAttemptedRef = useRef<string | null>(null)
  ...
  useEffect(() => {
    if (searchParams.get("tour") === "true") {
      ...
      router.replace(`/dashboard/workflows/${workflowId}`, { scroll: false })
    }
  ...
  useEffect(() => {
    if (workflow) {
      editor.loadWorkflow({
        ...
        category: (workflow as unknown as { category?: "TASK" | "CHATFLOW" | "AUTOMATION" }).category || "AUTOMATION",
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [workflow, workflowId, workflowsLoading])

  // Fallback: if hydration/list misses the target workflow, fetch it directly once.
  useEffect(() => {
    if (workflowsLoading || workflow) return
    if (workflowResolveAttemptedRef.current === workflowId) return

    workflowResolveAttemptedRef.current = workflowId
    setIsResolvingWorkflow(true)
    void getWorkflowById(workflowId)
      .catch(() => {
        // Keep not-found UI if direct fetch also fails.
      })
      .finally(() => {
        setIsResolvingWorkflow(false)
      })
  }, [workflow, workflowId, workflowsLoading, getWorkflowById])

  const handleToggleStatus = useCallback(async () => {
    const newStatus = editor.workflowStatus === "ACTIVE" ? "DRAFT" : "ACTIVE"
    try {
      await updateWorkflow(workflowId, { status: newStatus } as Partial<import("@/hooks/use-workflows").WorkflowItem>)
  ...
  }, [workflowId, editor, updateWorkflow])
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      const updated = await updateWorkflow(workflowId, {
        ...
        category: editor.workflowCategory as unknown as string,
      ...
      if (updated?.apiKey !== undefined) {
        editor.loadWorkflow({
          ...
          category: updated.category as "TASK" | "CHATFLOW" | "AUTOMATION" | undefined,
  ...
  }, [workflowId, deleteWorkflow, router])
  ...
  // Loading skeleton
  if (workflowsLoading || isResolvingWorkflow) {
    return (
