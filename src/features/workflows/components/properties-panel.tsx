...
import { X, ChevronDown, ChevronRight, AlertCircle, Plus, Trash2 } from "@/lib/icons"
...
function AssistantSelector({
  ...
  const assistants = initialAssistants || []
...
function CredentialSelector({
  ...
  initialCredentials?: CredentialOption[]
}) {
...
function WorkflowSelector({
  ...
  error?: string
  initialWorkflows?: { id: string; name: string; status: string }[]
}) {
...
function ToolSelector({
  ...
  const tools = useMemo(
    () =>
      (initialTools || []).filter((t) => (isMcp ? !t.isBuiltIn : true)),
    [initialTools, isMcp]
  )
...
function KnowledgeBaseGroupSelector({
  ...
  initialKnowledgeBaseGroups?: KBGroupOption[]
}) {
  const groups = initialKnowledgeBaseGroups || []
