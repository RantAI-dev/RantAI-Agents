...
import {
  ...
  findEnabledWidgetEmbedKey,
  findWidgetAssistantById,
  incrementWidgetEmbedKeyUsage,
...
// Handles widget chat messages (streaming)
export async function handleWidgetChat(req: NextRequest) {
  try {
    ...
    // Find the API key
    const embedKey = await findEnabledWidgetEmbedKey(apiKey)
    ...
    // Parse request body
    const parsedBody = WidgetChatBodySchema.safeParse(await req.json())
    ...
    const fileDocumentIds = body.fileDocumentIds
    const rawMessages = body.messages as RawWidgetMessage[]
    ...
    try {
      const chatflowWorkflow = await findActiveChatflowWorkflow(embedKey.assistantId)

      if (chatflowWorkflow) {
        console.log("[Widget Chat] Chatflow workflow found:", chatflowWorkflow.name)
        const lastMsg = [...rawMessages]
          .reverse()
          .find((m) => m.role === "user")
        const userText =
          lastMsg?.content ||
          ...
          if (streamBody && assistantMemoryConfig.enabled) {
            ...
            ;(async () => {
              try {
                ...
                if (isAnonymous) {
                  await setWidgetUserMemoryExpiry(
                    widgetUserId,
      ...
      try {
        ...
        if (widgetUserId !== 'anonymous') {
          ...
          if (isAnonymous) {
            ...
            const updateResult = await setWidgetUserMemoryExpiry(
              widgetUserId,
              expirationDate
            );
