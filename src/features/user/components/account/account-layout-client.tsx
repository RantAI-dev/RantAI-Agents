...
export default function AccountLayoutClient({
  ...
  }, [initialAvatarUrl, initialUser.email, initialUser.name, setProfile])

  const displayName = initialUser.name || "Agent"
  const displayEmail = initialUser.email || ""
  const displayUserId = initialUser.id || null
  const effectiveAvatarUrl = avatarUrl ?? initialAvatarUrl
    ...
    <div className="flex flex-col h-full bg-gradient-to-b from-panel-from via-panel-via via-[61%] to-panel-to">
      ...
      <div className="p-4 border-b border-sidebar-border">
        <div className="flex items-center gap-3 mb-3">
          <Avatar className="h-12 w-12 border-2 border-sidebar-border">
            {effectiveAvatarUrl && <AvatarImage src={effectiveAvatarUrl} alt={displayName} />}
      ...
      <div className="p-4 border-t border-sidebar-border">
        <div className="text-xs text-sidebar-muted space-y-1">
          ...
          <div className="flex justify-between">
            <span>User ID</span>
            <span className="font-mono">{displayUserId?.slice(0, 8) || "—"}...</span>
          </div>
        </div>
      </div>
    </div>
