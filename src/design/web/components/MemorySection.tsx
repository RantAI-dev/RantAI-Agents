...
function describeExtractionFailure(record: MemoryExtractionRecord): FriendlyExtractionFailure | null {
  ...
  const source =
    record.kind === 'connector'
      ? 'Connected apps were read, but RantAI Design could not turn that context into memory.'
      : 'RantAI Design could not run memory extraction for this chat.';
    ...
    try {
      ...
      if (friendlyFailure) {
        ...
        setConnectorError(
          ...
          ?? 'No memory suggestions found. RantAI Design could not read useful content from the selected app yet.',
        );
      } else {
        setConnectorStatus(
          ...
          <div className="memory-connector-workbench">
            <div className="memory-connector-picker-head">
              <div>
                ...
                <p className="hint">
                  Select connected apps first. RantAI Design only scans the apps you choose.
                </p>
              </div>
              <span className="memory-source-badge">
