...
export const en: Dict = {
  ...
  'chat.amrCard.switchBody': 'Switch to RantAI Design\'s official AMR model service — no API key setup needed. After you sign in, authorize, and top up, this run retries automatically.',
  ...
  'chat.runError.signInMessage.amr': 'The RantAI Design agent isn\'t signed in yet — sign in to start using it.',
  'chat.runError.signInMessage.other': '{agent} isn\'t signed in. Check its sign-in status locally. We recommend the RantAI Design agent — steadier and better value.',
  ...
  'settings.welcomeTitle': 'Welcome to RantAI Design',
  'settings.welcomeSubtitle': '',
  'settings.onboardingCreateTitle': 'Start from a brief',
  'settings.onboardingCreateBody': 'Describe the site, app, deck, image, or video you want. RantAI Design will create a project and keep the work editable.',
  ...
  'settings.onboardingMemoryCalloutBody': 'These answers seed your Memory profile. RantAI Design reuses it on every task — and keeps learning as you work.',
  ...
  'settings.onboardingGateTooltipAmr': 'The next steps run on AI — sign in to RantAI Design AMR to continue.',
  ...
  'settings.agentInstall.pathHint': 'If you installed a CLI with npm or Homebrew and it still shows as not installed, ensure the tool\'s bin directory is on the PATH the RantAI Design daemon inherits (Terminal vs GUI apps can differ on macOS). See QUICKSTART.md (section "Local agent CLI and PATH").',
  'settings.agentInstall.stepOpenLinks': 'Open Install or Docs for your preferred agent.',
  'settings.agentInstall.stepAuth': 'Authenticate with the vendor CLI (sign in or add API credentials) before returning to RantAI Design.',
  ...
  'settings.mcpServerHint': 'Expose RantAI Design as an MCP server for your coding agent.',
  ...
  'settings.privacyHint': 'What data is shared with the RantAI Design team',
  'settings.privacyConsentKicker': 'Help us improve RantAI Design',
  'settings.privacyConsentLead': 'RantAI Design can share usage data with our team to help us improve. This includes:',
  ...
  'settings.mcpHint': 'Let coding agents (Cursor, Claude Code, VS Code…) read your RantAI Design projects directly.',
  'settings.mcpDaemonError': 'Couldn\'t reach the local daemon to resolve install paths ({error}). Make sure RantAI Design is running, then reopen this panel.',
  ...
  'settings.mcpCapabilityDefault': 'Default to the project and file you have open in RantAI Design, so you can say "build this in my app" without re-stating which design.',
  'settings.mcpRunningNote': 'RantAI Design must be running for MCP tool calls to succeed. If you started your coding agent before opening RantAI Design, restart the agent so it can reach the live daemon.',
  ...
  'entry.discordAria': 'Join the RantAI Design Discord',
  'entry.discordAriaWithOnline': 'Join the RantAI Design Discord - {online}',
  'entry.discordOnlineLabel': '{count} online',
  'entry.followXLabel': 'Follow RantAI Design on X',
  ...
  'homeHero.chip.createPluginHint': 'Author a reusable RantAI Design plugin and add it to My plugins.',
  ...
  'handoff.promptIntro': 'Continue from this local RantAI Design project folder:',
  ...
  'designSystemPicker.noPreview': 'No preview page. Open the Design Systems panel to view the full preview.',
  ...
  'pluginsHome.subtitle': 'Ready-to-use RantAI Design workflows bundled with this runtime. Pick one to load a starter prompt, or browse the registry for more.',
  'pluginsHome.browseRegistry': 'Browse registry',
  'pluginsHome.count': '{filtered} of {total}',
  'pluginsHome.loadingCatalog': 'Loading catalog…',
  'pluginsHome.emptyCatalog': 'Catalog is empty. Bundled plugins ship with RantAI Design and should appear here automatically — try restarting the daemon if this persists.',
  ...
  'pluginCard.contributeAria': 'Contribute {title} to RantAI Design',
  'pluginCard.contributeTitle': 'Contribute plugin to RantAI Design with a pull request',
  'skillPluginCandidate.createForMe': 'Create plugin/template',
  'skillPluginCandidate.contributeToMain': 'Contribute to RantAI Design',
  'skillPluginCandidate.repoDescription': 'This repo looks like it could work as a plugin.',
  'integrations.kicker': 'Integration',
  'integrations.lede': 'Connect external systems, bring MCP tools into your agent loop, and use RantAI Design from other IDEs, scripts, and automations.',
  ...
  'mcpClient.daemonError': 'Could not reach the local daemon. Make sure RantAI Design is running, then reopen this panel.',
  'mcpClient.saveFailed': 'Save failed. Check that the daemon is running and try again.',
  'useEverywhere.modalAria': 'Use RantAI Design everywhere',
  'useEverywhere.modalTitle': 'Use RantAI Design everywhere',
  'useEverywhere.modalSubtitle': 'Drop RantAI Design into any IDE, agent, or script — CLI, HTTP, MCP, and Skills. Use “Copy guide for an agent” and paste into Claude Code, Codex, Cursor, openclaw, or hermes to set up everything.',
  ...
  'useEverywhere.section.overview.heading': 'RantAI Design works wherever your agent works',
  'useEverywhere.section.overview.intro': 'RantAI Design is more than a window — it is a local privileged daemon (`od`) plus a Skills + Design-Systems + Atoms registry. Once it is running on your machine, any code agent (Claude Code, Codex, Cursor, OpenCode/openclaw, Hermes, your own script) can drive generations, inspect projects, and produce design artifacts through four interchangeable surfaces.',
  'useEverywhere.section.overview.bullet1': 'CLI — `od <command>` for headless scripts, CI, and shell automation.',
  'useEverywhere.section.overview.bullet2': 'MCP server — wires RantAI Design as a Model Context Protocol server so any MCP-capable agent can list skills, run scenarios, and read artifacts.',
  ...
  'useEverywhere.section.overview.bullet4': 'Skills — drop-in `SKILL.md` packs (Claude-compatible) that any agent already on your PATH can invoke without RantAI Design at all.',
  ...
  'useEverywhere.section.cli.heading': 'Drive RantAI Design from any shell',
  ...
  'useEverywhere.section.mcp.heading': 'Expose RantAI Design as an MCP server to any coding agent',
  'useEverywhere.section.mcp.intro': 'RantAI Design ships with a Model Context Protocol server (`od mcp`) that lets any MCP-capable client — Cursor, Claude Code, Antigravity, VS Code Copilot Chat, openclaw, hermes — discover RantAI Design tools (list skills, render previews, generate media, run plugins) without shelling out manually. The daemon publishes a ready-to-paste install snippet via `GET /api/mcp/install-info` for each major client.',
  ...
  'useEverywhere.section.mcp.footer': 'In the RantAI Design app, open Settings → Integrations to copy a client-specific install command (Cursor, Claude Code, Antigravity, VS Code) instead of editing JSON by hand.',
  ...
  'useEverywhere.section.skills.heading': 'Drop-in Skills for any agent — even without RantAI Design running',
  'useEverywhere.section.skills.intro': 'A Skill is a directory with a Claude-compatible `SKILL.md` (YAML front-matter + body). RantAI Design extends the format with the `od:` namespace (`mode`, `preview`, `design_system`, `inputs`, …) so the same artifact can be used both inside RantAI Design and by a vanilla agent like Claude Code, Codex, openclaw, or hermes. Discovery follows a precedence chain so projects can override their own skills.',
  ...
  'useEverywhere.section.skills.snippet1': 'Minimal SKILL.md (Claude-compatible front matter + RantAI Design extras)',
  ...
  'entry.githubStarAria': 'Star RantAI Design on GitHub',
  ...
  'newproj.locationDefault': 'RantAI Design projects',
  ...
  'socialShare.openDesignSection': 'Share RantAI Design',
  'socialShare.openDesignTitle': 'RantAI Design on GitHub',
  'socialShare.openDesignText': 'RantAI Design is an open-source workspace for creating, editing, deploying, and handing off design artifacts.',
  ...
  'socialShare.projectText': 'Built with RantAI Design: {title}. View the deployed artifact and star the repo: {repo}',
  'socialShare.projectCopyText': 'Built with RantAI Design: {title}\n{url}\nRantAI Design repo: {repo}',
  ...
  'brandEnrichment.body': "RantAI Design will run a deeper extraction in the background, apply the right design-system skills automatically, and update this system in place. This can take 10-20 minutes; you can keep working.",
  ...
  'chat.activeFilePlaceholder': 'Ask RantAI Design to change {file}...',
  ...
  'chat.mode.design.solves': 'Use it when you want RantAI Design to generate files, update the workspace, or turn a brief into multimodal deliverables.',
  ...
  'preview.errorBody': 'The example HTML failed to fetch. Make sure RantAI Design is running and try again.',
  'preview.retry': 'Retry',
  ...
  'designFiles.usefulInfoTip7': 'Enjoying RantAI Design? Star us on GitHub.',
  'designFiles.usefulInfoTip8': 'Follow RantAI Design on X for the latest updates.',
  ...
  'assistant.shareToOpenDesign': 'Share to RantAI Design',
  ...
  'pet.tabBuiltInHint': 'Curated companions bundled with RantAI Design — pick one and adopt.',
  ...
  'pet.codexBundledTitle': 'Ships with RantAI Design — no download needed.',
  ...
  'settings.projectLocationsDescription': 'Add work bases that can contain multiple RantAI Design project folders. New projects are saved as one folder inside the selected base.',
  ...
  'settings.customInstructionsHint': 'Fixed instructions RantAI Design should follow in every chat. Save facts, preferences, and project context as memories.',
  ...
  'updater.availableBody': 'RantAI Design {version} is available. It will download before the installer can open.',
  ...
  'updater.installerOpenBody': 'The installer is open. RantAI Design is quitting so you can finish the update.',
  ...
  'updater.quitButton': 'Quit RantAI Design',
  'updater.quitFailedBody': 'The installer is open, but RantAI Design could not quit. Quit RantAI Design before replacing the app.',
  'updater.quitFailedTitle': 'Could not quit',
  'updater.quitting': 'Quitting...',
  'updater.ready': 'Update ready',
  'updater.readyGeneric': 'A new version is ready. RantAI Design will close and open the installer.',
  'updater.readyVersion': 'RantAI Design {version} is ready. RantAI Design will close and open the installer.',
  ...
  'dsManager.createBody': 'Teach RantAI Design your brand, product, code, assets, and design references.',
  ...
  'settings.customInstructionsDesc': 'Fixed instructions RantAI Design follows in every chat. These are not saved memories; use Memory for facts, preferences, and project context.',
  ...
  'brand.librarySubtitle': 'Teach RantAI Design your design systems — extract one from a website, then apply it in any chat.',
  ...
  'onboarding.buildBody': 'RantAI Design turns your brand into a reusable design system — then keeps everything you build on-brand and consistent.',
