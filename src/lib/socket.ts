  ...
  io.on("connection", (socket) => {
    ...
    if (socket.data.isExternalApi) {
      socket.on("api:chat", async ({ messages, stream }: { messages: Array<{ role: string; content: string }>; stream?: boolean }) => {
        try {
          const { runV1ChatCompletion, authenticateV1Request } = await import("@/features/agent-api/service")
          const { V1ChatCompletionSchema } = await import("@/features/agent-api/schema")
